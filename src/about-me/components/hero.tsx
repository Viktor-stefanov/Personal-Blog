...
export function Hero() {
  const { whoami, now, headline, paragraphs, actions, portrait, reading, quote } =
    ABOUT;
      ...
      <Container>
        ...
        <div className="grid items-start gap-3xl lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="flex flex-col gap-2xl">
            <Terminal>
              <TermLine prompt>whoami --verbose</TermLine>
              <TermLine tone="out">
                {whoami.name} <span className="text-muted">{whoami.rest}</span>
              </TermLine>
              <TermLine prompt>cat now.txt</TermLine>
              <TermLine tone="muted">{now}</TermLine>
              <TermLine prompt>
                <Cursor />
              </TermLine>
            </Terminal>
